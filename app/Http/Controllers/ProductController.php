...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (auth()->user() && auth()->user()->isAbleTo('Manage Products')) {
            ...
            $products = Product::where('theme_id', $store_id->theme_id)->where('product_type', null)->where('store_id', getCurrentStore())->orderBy('id', 'desc')->get();
                ...
                if ($request->variant_product == 0) {
                    if ($total_products < $plan->max_products || $plan->max_products == -1) {
                        ...
                        if (module_is_active('CartQuantityControl')) {
                            \Modules\CartQuantityControl\app\Models\CartQuantityControl::saveData($Product, $request->minimum_quantity, $request->maximum_quantity);
                        }

                        $Product->save();
                        if (module_is_active('PartialPayments')) {
                            \Modules\PartialPayments\app\Models\PartialPayments::saveData($Product,$request->all());
                        }
                        if (module_is_active('SizeGuideline')) {
                            \Modules\SizeGuideline\app\Models\SizeGuideline::saveData($Product, $request->size_chart_title, $request->size_chart_information,$request->all());
                        }
                        if (module_is_active('WholesaleProduct')) {
                            \Modules\WholesaleProduct\app\Models\WholesaleProduct::saveData($Product, $request->wholesale_repeater_basic);
                        }
                            ...
                            if (count($a_option[0]) > 0) {
                                ...
                                foreach ($a_option as $key => $com) {
                                    ...
                                    foreach ($com as $key => $item) {
                                        ...
                                        if (module_is_active('CartQuantityControl')) {
                                            \Modules\CartQuantityControl\app\Models\CartQuantityControl::variantSaveData($request, $product_stock, $str);
                                        }
    ...
    public function update(Request $request, Product $product)
    {
        if (auth()->user() && auth()->user()->isAbleTo('Edit Products')) {
            ...
            if (module_is_active('SizeGuideline')) {
                \Modules\SizeGuideline\app\Models\SizeGuideline::saveUpdateData($product, $request->size_chart_title,$request->size_chart_information,$request->all());
            }
            if (module_is_active('WholesaleProduct')) {
                \Modules\WholesaleProduct\app\Models\WholesaleProduct::saveUpdateData($product, $request->wholesale_repeater_basic);
            }
            if (module_is_active('ProductBarCode')) {
                \Modules\ProductBarCode\app\Models\ProductBarCode::saveData($product);
            }
    ...
    public function product_price(Request $request, $slug)
    {
        ...
        if (!empty($product)) {
            if (module_is_active('WholesaleProduct')) {
                if (!module_is_active('SizeGuideline')) {
                    if (module_is_active('WholesaleProduct')) {
                        $return = \Modules\WholesaleProduct\app\Models\WholesaleProduct::Wholesale_Product_price($product, $request->all(),$store);
                    }else{
                    ...
                    if (module_is_active('SizeGuideline')) {
                        $return = \Modules\SizeGuideline\app\Models\SizeGuideline::Size_Product_price($product, $request->all(),$store);
                    }
                }
                ...
                foreach ($products as $key => $product) {
                    ...
                    if ($product->variant_product != '1') {
                        ...
                        if ($request->session_key == 'purchases') {
                            ...
                            $productprice = Product::ProductPrice($store_id->theme_id, $store_id->slug, $product->id,0);
    ...
    public function addToCart(Request $request, $id, $session_key, $variant_id = 0)
    {
        ...
        if (isset($request->variants)) {
            ...
            if ($variant) {
                ...
                if ($session_key == 'pos_' . getCurrentStore()) {
                    ...
                    $productprice = Product::ProductPrice($store_id->theme_id, $store_id->slug, $product->id,$variant->id);
        ...
        $tax_option = \App\Models\TaxOption::where('store_id', $store_id->id)
        ...
        $carthtml .= '<tr data-product-id="' . $id . '" id="product-id-' . $id . '">
                        ...
                        <td class="tax">' . (!empty($product_tax) ? $product_tax : '-') . '</td>
