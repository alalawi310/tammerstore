@extends('layouts.app')
@php $currency_icon = \App\Models\Utility::GetValueByName('CURRENCY', APP_THEME(), auth()->user()->current_store); @endphp
                                    ...
                                    @if (session($lastsegment) && !empty(session($lastsegment)) && count(session($lastsegment)) > 0)
                                        @foreach (session($lastsegment) as $id => $details)
                                            @php
                                                ...
                                                $tax_option = \App\Models\TaxOption::where('store_id', auth()->user()->current_store)
                                            ...
                                            @if (auth()->user()->current_store == $product->store_id)
                                                <tr data-product-id="{{ $id }}"
                                                    ...
                                                    <td class="price text-center">
                                                        {{ currency_format_with_sym($details['orignal_price'], auth()->user()->current_store, APP_THEME()) }}
                                                    </td>

                                                    <td class="text-center">
                                                        <span
                                                            class="total_orignal_price">{{ currency_format_with_sym($details['total_orignal_price'], auth()->user()->current_store, APP_THEME()) }}</span>
                                                    </td>
                                                    <td>
                                                        {!! Form::open([
                                                            'method' => 'DELETE',
                                    ...
                                    <div class="right-inner mt-3">
                                        <div class="d-flex text-end justify-content-md-end  justify-content-flex-start">
                                            ...
                                            <h6 class="mb-0 text-dark subtotal_price" id="displaytotal">
                                                {{ currency_format_with_sym($total, auth()->user()->current_store, APP_THEME()) ?? SetNumberFormat($total) }}
                                            </h6>
                                        </div>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.code == '200') {
                        ...
                        $('#displaytotal').text(addCommas(data.product
                        .total_orignal_price));
                    ...
                    if ('carttotal' in data) {
                        $.each(data.carttotal, function(key, value) {
                            $('#product-id-' + value.id + ' .total_orignal_price').text(
                                addCommas(value.total_orignal_price));
                            sum += value.total_orignal_price;
                        });
                        $('#displaytotal').text(addCommas(sum));
