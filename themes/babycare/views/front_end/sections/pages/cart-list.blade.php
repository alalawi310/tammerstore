...
<div class="container">
    <div class="section-title">
        ...
        <h2> {!! $page_json->cart_page->section->title->text ?? __('Shopping category') !!} <span>({{ $response->data->cart_total_product }})</span></h2>
    </div>
    ...
    <div class="row">
                        ...
                        @if (!empty($response->data->cart_total_product))
                            @foreach ($response->data->product_list as $item)
                                <tr>
                                    ...
                                    <td data-label="Name">
                                        <a
                                            href="{{ url($slug . '/product/' . getProductSlug($item->product_id)) }}">{{ $item->name }}</a>
                                    ...
                                    <td data-label="quantity">
                                        <div class="qty-spinner">
                                            <button type="button" class="quantity-decrement change-cart-globaly"
                                                cart-id="{{ $item->cart_id }}" quantity_type="decrease">
                                            ...
                                            <input type="text" class="quantity 45_quatity"
                                                data-cke-saved-name="quantity" name="quantity"
                                                value="{{ $item->qty }}" min="01"
                                                id="cart_list_quantity{{ $item->qty }}" disabled>

                                            <button type="button" class="quantity-increment change-cart-globaly"
                                                cart-id="{{ $item->cart_id }}" quantity_type="increase">
                                    ...
                                    <td>
                                        ...
                                        <button class="btn remove_item_from_cart" title="Remove item"
                                            href="JavaScript:void(0)" data-id="{{ $item->cart_id }}">
        ...
        <div class="col-lg-3 col-12">
            <div class="cart-summery">
                <ul>
                    <li>
                        <span class="cart-sum-left">{{ $response->data->cart_total_product }}
                            {{ __('item') }}</span>
                        <span
                            class="cart-sum-right">{{ currency_format_with_sym($response->data->final_price ?? ($response->data->sub_total ?? 0), $store->id, $currentTheme) ?? SetNumberFormat($response->data->final_price ?? ($response->data->sub_total ?? 0)) }}</span>
                    </li>
                    <li>
                        <span class="cart-sum-left">{{ __('Taxes') }}: </span>
                        <span
                            class="cart-sum-right">{{ currency_format_with_sym($response->data->tax_price, $store->id, $currentTheme) ?? SetNumberFormat($response->data->tax_price) }}</span>
                    </li>
                    <li>
                        <span class="cart-sum-left">{{ __('Discount') }}: </span>
                        <span
                            class="cart-sum-right coupon_discount_amount">{{ currency_format_with_sym(0, $store->id, $currentTheme) ?? SetNumberFormat(0) }}</span>
                    </li>
                    @php
                    ...
                    <li>
                        ...
                        <span
                            class="cart-sum-right discount_final_price">{{ currency_format_with_sym($final, $store->id, $currentTheme) ?? SetNumberFormat($final) }}</span>
                    </li>
                </ul>
