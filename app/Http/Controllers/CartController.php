...
class CartController extends Controller
{
    public function cart_list_sidebar(Request $request, $storeSlug)
    {
        ...
        if (auth('customers')->guest()) {
            ...
            $request->merge(['customer_id' => auth('customers')->user()->id, 'store_id' => $store->id, 'slug' => $slug,'theme_id' => $currentTheme,'status' => true]);
        ...
        if ($response->status == 1) {
            ...
            if (module_is_active('SkipCart')) {
                ...
                if($plan && strpos($plan->modules, 'SkipCart') !== false)
                {
                    ...
                    {
                        $return['sticky_product_list'] = view('skipcart::theme.sticky-product-list', compact('response', 'currency', 'currency_name', 'currentTheme', 'store','slug'))->render();
                    }
                }
            }
    ...
    public function product_cartlist(Request $request, $slug)
    {
        ...
        $request->request->add(['variant_id' => $variant_id,'product_id' => $product_id, 'customer_id' => $customer_id, 'qty' => $qty, 'store_id' => $store->id, 'slug' => $slug, 'theme_id' => $theme_id]);
        $api = new ApiController();
