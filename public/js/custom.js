...
$(document).ready(function () {
    ...
    function updateContainerTop() {
        // var dasHeaderHeight = $('.dash-header .header-wrapper').outerHeight(); // Get the height of the header
        // $('.dash-container').css('top', dasHeaderHeight + 'px'); // Apply the top position
    }
...
function comman_function() {
    ...
    if ($(".summernote-simple").length) {
        // Get the raw content from the textarea and sanitize it
        var content = $('.summernote-simple').val();
        var sanitizedContent = sanitizeScriptTags(content);
        
        // Set the sanitized content back to the textarea before initializing Summernote
        $('.summernote-simple').val(sanitizedContent);
        $('.summernote-simple').summernote({
            ...
            toolbar: [
             ...
             // Sanitize the content on initialization (onLoad) and on changes
            callbacks: {
                onInit: function() {
                    // On initialization, sanitize the content
                    var contents = $('.summernote-simple-product').val();
                    var sanitizedContent = sanitizeScriptTags(contents);
                    $('.summernote-simple').val(sanitizedContent); // Set sanitized content
                },

                onChange: function(contents) {
                    // On change, sanitize the content to prevent scripts from being saved
                    var sanitizedContent = sanitizeScriptTags(contents);
                    $('.summernote-simple').val(sanitizedContent); // Set sanitized content
                }
            }
    ...
    // Function to sanitize script tags
    function sanitizeScriptTags(content) {
        return content.replace(/<script.*?>.*?<\/script>/g, function(match) {
            return '&lt;script&gt;' + match.replace(/<\/?script>/g, '') + '&lt;/script&gt;';
        });
    }
// use for Product
    if ($(".summernote-simple-product").length) {
        // Get the raw content from the textarea and sanitize it
        var content = $('.summernote-simple-product').val();
        var sanitizedContent = sanitizeScriptTags(content);
        
        // Set the sanitized content back to the textarea before initializing Summernote
        $('.summernote-simple-product').val(sanitizedContent);
        $('.summernote-simple-product').summernote({
            ...
            callbacks: {
                onInit: function() {
                    ...
                    var contents = $('.summernote-simple-product').val();
                    var sanitizedContent = sanitizeScriptTags(contents);
                    $('.summernote-simple-product').val(sanitizedContent); // Set sanitized content
                },
