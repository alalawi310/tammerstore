...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (auth()->user() && auth()->user()->isAbleTo('Manage Order')) {
            ...
            $orders = Order::where('theme_id', $store_id->theme_id)->where('store_id', getCurrentStore())->orderBy('created_at','desc')->get();
    ...
    public function order_summary(Request $request, $slug)
    {
        ...
        elseif((isset($request['data']['order_id']) && !empty($request['data']['order_id'])) || (isset($request->responce['data']['order_id']) && ($request->responce['data']['order_id'])) || (isset($order_id) && !empty($order_id)) )
        {
            ...
            return view('front_end.sections.pages.order-summary',compact('store','order_data','slug','currentTheme','currantLang','currency','languages','section','order_complate_description','order_complate_title','MainCategoryList')+$sqlData+$data);
        }
        else{
    ...
    public function order_status_change(Request $request)
    {
        ...
        if (Module::has('ReviewReminder') && Module::isEnabled('ReviewReminder')) {
        ...
        try
        {
            ...
            $msg = __("Hello, Welcome to $store->name .Your Order is $request->delivered, !Hi $d_order->product_order_id, Thank you for Shopping");
