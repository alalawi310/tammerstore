                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @elseif ($item->customer_id != 0)
                                                {{ !empty($item->CustomerData->name) ? $item->CustomerData->name : '' }}<br>
                                        ...
                                        <td>
                                            ...
                                            {{ __('FedPay')}}
                                            @elseif ($item->payment_type == 'CinetPay')
                                            {{ __('CinetPay')}}
                                            @elseif ($item->payment_type == 'SenagePay')
                                            {{ __('SenagePay')}}
                                            @elseif ($item->payment_type == 'CyberSource')
                                            {{ __('CyberSource')}}
                                            @elseif ($item->payment_type == 'Ozow')
                                            {{ __('Ozow')}}
                                            @elseif ($item->payment_type == 'MyFatoorah')
                                            {{ __('MyFatoorah')}}
                                            @elseif ($item->payment_type == 'easebuzz')
                                            {{ __('Easebuzz')}}
                                            @elseif ($item->payment_type == 'NMI')
                                            {{ __('NMI')}}
                                            @elseif ($item->payment_type == 'PayU')
    				                        {{ __('PayU')}}
                                            @elseif ($item->payment_type == 'sofort')
                                            {{ __('Sofort')}}
                                            @elseif ($item->payment_type == 'esewa')
                                            {{ __('Esewa')}}
                                            @elseif ($item->payment_type == 'Paynow')
                                            {{ __('Paynow')}}
                                            @elseif ($item->payment_type == 'DPO')
                                            {{ __('DPO')}}
                                            @elseif ($item->payment_type == 'Braintree')
                                            {{ __('Braintree') }}
                                            @elseif ($item->payment_type == 'PowerTranz')
                                            {{ __('PowerTranz') }}
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            @elseif ($item->delivered_status == 7)
                                                <button type="button"
                                                    class="btn btn-sm btn-soft-dark btn-icon bg-dark rounded-pill badge-same">
                                        ...
                                        <td class="text-end">
                                            <div class="d-flex gap-1 justify-content-end">
                                                ...
                                                <a href="#"
                                                    class="btn btn-sm btn-primary return_request" data-id="{{ $item->id }}" data-status="2" data-bs-toggle="tooltip"
                                                    title="{{__('Return order request approve') }}">
                                                    <i class="ti ti-check"></i>
                                                </a>
                                                <a href="#"
                                                    class="btn btn-sm btn-danger return_request" data-id="{{ $item->id }}" data-status="3" data-bs-toggle="tooltip"
                                                    title="{{__('Return order request cancel') }}">
                                                    <i class="ti ti-circle-x"></i>
                                                </a>
                                                @endif

                                                {{-- @permission('Show Orders') --}}
                                                    <a href="javascript:void(0)" data-url="{{ route('order.order_view', \Illuminate\Support\Facades\Crypt::encrypt($item->id)) }}" data-size="lg" data-ajax-popup="true" data-title="{{__('Order')}}    #{{ $item->product_order_id }}" class="x-3 btn btn-sm align-items-center btn btn-sm btn-warning" data-bs-toggle="tooltip" data-original-title="{{__('Show')}}" data-bs-toggle="tooltip"
                                                    title="{{__('Show') }}">
                                                        <i class="ti ti-eye"></i>
                                                    </a>
                                                    <a href="{{ route('order.view', \Illuminate\Support\Facades\Crypt::encrypt($item->id)) }}"
