...
if(!function_exists('getHomePageDatabaseSectionDataFromDatabase')){
    function getHomePageDatabaseSectionDataFromDatabase($data)
    {
        ...
        return compact('categories','latest_product', 'search_products', 'MainCategoryList', 'SubCategoryList', 'reviews', 'pages', 'category_id', 'has_subcategory', 'discount_products', 'products', 'all_products', 'modern_products', 'home_products', 'home_page_products', 'landing_product', 'random_product', 'bestSeller', 'all_slider_products');
    }
}
                ...
                if (request()->route('storeSlug') || request()->route('slug')) {
                    ...
                    if ($store) {
                        ...
                        if ($user) {
                            ...
                            if ($plan && strpos($plan->modules, $module) !== false) {
                                return true;
                            }
...
if (!function_exists('ActivatedModule')) {
    function ActivatedModule($user_id = null)
    {
        ...
        return $user_active_module ?? null;
    }
}
...
if (!function_exists('getInnerPageJson')) {
    function getInnerPageJson($theme_id, $store_id, $specific_page_name = null) {
        $page_names = ['order-complete', 'product_page', 'blog_page', 'cart_page', 'checkout_page', 'contact_page', 'faq_page', 'collection_page', 'article_page']; // Correct the typo
        $default_paths = [
