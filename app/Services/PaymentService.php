...
class PaymentService
{
    ...
    private function processStripe($request, $slug, $response)
    {
        ...
        if ($totalprice > 0.0) {
            ...
            if (module_is_active('CheckoutAttachment')) {
                \Modules\CheckoutAttachment\app\Models\CheckoutAttachment::CheckoutAttachmentStore($slug, $request);
            }
            $data = $data ?? false;
            try {
    ...
    private function processToyyibPay($request, $slug, $response)
    {
        try
        {
            ...
            $toyyibpay_secret_key = \App\Models\Utility::GetValueByName('toyyibpay_secret_key',$theme_id, $store->id);
            $toyyibpay_category_code = \App\Models\Utility::GetValueByName('toyyibpay_category_code',$theme_id, $store->id);
            $CURRENCY = \App\Models\Utility::GetValueByName('CURRENCY',$theme_id, $store->id);
            if (is_array($request->get('billing_info'))) {
                $email = $request->get('billing_info')['email'];
                $firstname = $request->get('billing_info')['firstname'];
    ...
    public function processPhonePe($request, $slug, $response)
    {
        ...
        $response = $provider->makePayment(
            ...
            route('store.payment.phonepe', $slug),
            $orderID
        );
    ...
    public function processEasebuzz($request, $slug, $response)
    {
        ...
        $requests_data = $request->except('attachment');
        // $requests_data = $request->all();
    ...
    public function processDPO($request, $slug, $response)
    {
        ...
        try {
            ...
            return redirect()->route('store.dpo.view', ['slug' => $slug, 'theme_id' => $theme_id, 'store_id' => $store->id,
                'total_price' => $totalprice, 'user' => $user, 'action' => route('store.pay.with.dpo')]);
        } catch (\Exception $ex) {
    ...
    public function processPowerTranz($request, $slug, $response)
    {
        ...
        try {
            ...
            return redirect()->route('store.Powertranz.view',['slug' => $slug ,'amount' => $cartlist_final_price,'theme_id' => $theme_id,'store_id' => $store->id,
            'currantLang' => $currantLang,'currentTheme' => $currentTheme,'user' => $user]);
        } catch (\Exception $ex) {
